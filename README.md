...
RAPIDWIFI-ZONE is a voucher-based hotspot management system built with Node.js, Express, SQLite, and MikroTik integration. It provides secure voucher lifecycle management, operator dashboards, admin dashboards, audit logs, analytics, and payment integration.

---

## Features

- **Voucher Login**: Users authenticate with vouchers generated by operators or self-service.
- **Admin Dashboard**:
  - Create vouchers with batch tags and profile selection.
  - Export vouchers (CSV/JSON).
  - View audit logs and payments.
  - Bulk actions (activate, block, delete) with expired voucher restriction.
  - Operator/Admin management page with single and bulk actions.
  - Analytics dashboard (voucher status, payments trend, revenue trend, profile revenue breakdown).
- **Operator Dashboard**:
  - Sell vouchers tied to MikroTik profiles.
  - Record cash payments with validation.
  - Daily sales tracking.
- **Self-Service Payment**:
  - Users can purchase vouchers via mobile money.
- **Audit Logs**:
  - Track voucher lifecycle and admin/operator actions.
- **Analytics**:
  - Voucher status chart (active/inactive).
  - Payments trend chart (daily counts).
  - Revenue trend chart (daily sums).
...
```bash
git clone https://github.com/yourrepo/rapidwifi-zone.git
cd rapidwifi-zone
npm install
...
- **Admin Dashboard**:
  ...
  - View audit logs and payments.
  - Bulk actions (activate, block, delete) with expired voucher restriction.
  - Operator/Admin management page.
  - Analytics dashboard (active vs inactive vouchers).
- **Operator Dashboard**:
  - Sell vouchers tied to MikroTik profiles.
  - Record cash payments with validation.
  - Daily sales tracking.
...
- **Analytics**:
  - Voucher status chart (active/inactive).
  - Revenue trend chart (extendable).
- **Security**:
  - CSRF protection.
  - Session management.
  - Role-based access control.
