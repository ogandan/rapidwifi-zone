...
async function ros(cmd) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const connOpts = {
      ...
      algorithms: {
        serverHostKey: (process.env.ROS_SERVER_HOST_KEY_ALGOS || 'ssh-rsa').split(',').map(s => s.trim()).filter(Boolean),
        publicKey: (process.env.ROS_PUBLIC_KEY_ALGOS || 'ssh-rsa').split(',').map(s => s.trim()).filter(Boolean)
      },
