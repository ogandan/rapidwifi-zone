...
// Purpose: Voucher lifecycle manager with TEST_MODE simulation and RouterOS SSH support

require('dotenv').config();
const fs = require('fs');
const path = require('path');
const crypto = require('crypto');
const { Client: SSHClient } = require('ssh2'); // keep ssh2
...
async function ros(cmd) {
  if (TEST_MODE) {
    ...
    // Simulate block (disabled=yes)
    if (cmd.includes('disabled=yes')) {
      // Extract username from [find name=...]
